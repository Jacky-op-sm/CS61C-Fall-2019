...
mapLoop:
    #add t1, s0, x0     # Mistake 2: should load word ops, but not addi
    lw t1, 0(s0)        # load the address of the array of current node into t1
    lw t2, 4(s0)        # load the size of the node's array into t2

    # t0 is the counter shall be changed every loop

    slli t3, t0, 2      # Mistake 1: int takes 4 bytes, should shift left by 2
    add t1, t1, t3      # offset the array address by the count
    lw a0, 0(t1)        # load the value at that address into a0

    # t1, t2, t3 is temperory register for callee, for callor we should store it first, only then 
    # can we use them later.
    # Reason? Because they may be changed!
    addi sp, sp, -12    # Mistake 4: save for temperory register.
    sw t1, 0(sp)
    sw t2, 4(sp)
    sw t3, 8(sp)
    ...
    #la a0, 8(s0)       # Mistake 3: la ops follows an immediate
    lw a0, 8(s0)        # load the address of the next node into a0
    #lw a1, 0(s1)       # Mistake 5: s1 is already the address of the function
                        # And s1 is not an array!!! can't use indentation  
